...
async function validateTimesheetState(timesheetId: string) {
  ...
  if (timesheet.status !== 'PENDING') {
    return {
      isValid: false,
      error: `Timesheet status is ${timesheet.status}. Only PENDING timesheets can be approved.`,
      timesheet: null,
    };
  }
 ...
 *
 * Approve a pending timesheet (Manager only)
 *
...
export async function POST(
  ...
  try {
    ...
    console.log('Timesheet approved successfully', {
      ...
      timesheetId,
      approverId: user.id,
      playerName: `${timesheet.player?.firstName} ${timesheet.player?.lastName}`,
      approverName,
